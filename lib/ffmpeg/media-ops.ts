...
import { type FFmpegProgressCallback, getFFmpeg } from './loader';
...
export async function mergeMedia(
  ...
  // Consumers expect one continuous 0..1 progress stream, even though work runs per-file.
  ...
  // Normalize inputs first so concat can run losslessly and avoid timeline drift.
  ...
  // Use stream copy concat once streams are aligned to keep merge fast and deterministic.
