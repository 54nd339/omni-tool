...
export function ChangelogModal() {
  ...
  const [dismissedWithoutAck, setDismissedWithoutAck] = useState(false);

  const hasNewVersion = lastSeenVersion !== LATEST_VERSION;
  const isOpen =
    changelogModalOpen || (hasNewVersion && !dismissedWithoutAck);

  useEffect(() => {
    if (hasNewVersion) {
      setChangelogModalOpen(true);
      setDismissedWithoutAck(false);
    }
  ...
  const handleOpenChange = (open: boolean) => {
    if (!open) {
      setChangelogModalOpen(false);
      if (hasNewVersion) setDismissedWithoutAck(true);
    }
  };
