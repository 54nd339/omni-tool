...
import { QR_ERROR_CORRECTION_LEVELS } from '@/lib/constants/generators';
import {
  generateQrPngBlob,
  generateQrSvg,
...
import type { ErrorCorrectionLevel } from '@/types/common';
...
export function QrGeneratorTool() {
  ...
  const [params, setParams] = useQueryStates({
    ...
    ec: parseAsString.withDefault('M'),
  }, { shallow: true });

  const { text, size, fg: fgColor, bg: bgColor, ec } = params;
  const ecLevel: ErrorCorrectionLevel = isErrorCorrectionLevel(ec) ? ec : 'M';
        ...
        <div className="space-y-6">
          <div>
            ...
            <ToggleGroup
              ...
              value={ecLevel}
              onValueChange={(value) => {
                if (value && isErrorCorrectionLevel(value)) {
                  setParams({ ec: value });
                }
              }}
            >
              {QR_ERROR_CORRECTION_LEVELS.map((l) => (
